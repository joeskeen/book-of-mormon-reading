...
function onSubmit(e) {
  ...
  results.innerText = `To read the Book of Mormon between ${format(startDate)} and ${format(endDate)} (${days} days), you will need to read ${Math.ceil(ppd)} pages per day.`;

  const currentDay = Math.ceil((now.getTime() - startDate.getTime()) / ONE_DAY);
